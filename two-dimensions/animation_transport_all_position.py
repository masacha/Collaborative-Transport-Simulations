...
data = np.genfromtxt('simulation_transport_all_position.dat',
                     names=True,
                     dtype=None,
                     delimiter=' ')
...
right1 = plt.Circle((5, -5), 0.08, fc='r', alpha=0.5)
...
def animate(i):
    ...
    left2.center = (data[frame_skip*i][7], data[frame_skip*i][8])
    left2front.center = (data[frame_skip*i][7]+np.cos(data[frame_skip*i][9])*0.08, data[frame_skip*i][8]+np.sin(data[frame_skip*i][9])*0.08)
    right1.center = (data[frame_skip*i][10], data[frame_skip*i][11])
    right1front.center = (data[frame_skip*i][10]+np.cos(data[frame_skip*i][12])*0.08, data[frame_skip*i][11]+np.sin(data[frame_skip*i][12])*0.08)
    return left1, left1front, left2, left2front, right1, right1front, rectangle,
