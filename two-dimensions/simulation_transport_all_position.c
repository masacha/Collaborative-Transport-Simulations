...
/*OBJECT*/
	...
	if((fp=fopen("simulation_transport_all_position.dat", "w"))==NULL){
	...
	while(t <= T){	
		...
		x_o_ref += v_t*cos(phi_o_ref)*ST;
		y_o_ref += w_t*sin(phi_o_ref)*ST;
		phi_o_ref += w_t*ST;
		...
		beta_left1 = - (phi_res_left1-phi_ref_left1) - alpha_left1;

		cmd_velocity_left1 = K_ro*ro_left1;
		cmd_angular_velocity_left1 = K_alpha*alpha_left1+K_beta*beta_left1;
		...
		beta_left2 = - (phi_res_left2-phi_ref_left2) - alpha_left2;

		cmd_velocity_left2 = K_ro*ro_left2;
		cmd_angular_velocity_left2 = K_alpha*alpha_left2+K_beta*beta_left2;
		...
		x_ref_right1 = x_o_ref + cos(phi_o_ref)*(+L_o+R_r)-sin(phi_o_ref)*(0.0);
		y_ref_right1 = y_o_ref + sin(phi_o_ref)*(+L_o+R_r)+cos(phi_o_ref)*(0.0);
		phi_ref_right1 = phi_o_ref;
		...
		beta_right1 = - (phi_res_right1-phi_ref_right1) - alpha_right1;

		cmd_velocity_right1 = K_ro*ro_right1;
		cmd_angular_velocity_right1 = K_alpha*alpha_right1+K_beta*beta_right1;
